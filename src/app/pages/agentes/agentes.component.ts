...
export class AgentesComponent implements OnInit {
    ...
    titulopant : string = "Agente";
    icono : string = "pe-7s-next-2";
    loading: boolean = false;
    exportarHabilitado: boolean = false;

    @ViewChild('OpenModalEquipoAgente', { static: false }) OpenModalEquipoAgente!: TemplateRef<any>;
    ...
    AsignarEquipo(age_id: number, age_activo: number) {
      this.selectedAgente = this.dataAgente.find(x => x.age_id === age_id);
      this.modalRef = this.modalService.show(this.OpenModalEquipoAgente, { class: 'modal-lg modal-dialog-centered'});
   }
    ...
    loadDataProceso() {
      ...
      this.api.getagenteman(data_post).subscribe({
        next: (data: any[]) => {
          if (Array.isArray(data) && data.length > 0) {
            this.dataAgente = data.map(item => ({
              ...item,
    ...
    AnularAgente(age_id,age_activo) {
      const texto = age_activo == 1 ? "¿Seguro de <b>anular</b> al Agente?" : "¿Seguro de <b>activar</b> al Agente?"

      const dataPost = {
          p_age_id:(age_id == null || age_id === '') ? 0 : parseInt(age_id),
          p_age_anular: age_activo == 1 ? 1 : 0,
          p_age_usureg:parseInt(localStorage.getItem("usuario"))
        };
        swal.fire({
          title: 'Mensaje',
          html: texto,
          ...
          if (result.isConfirmed) {
            this.api.getagenteanu(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
