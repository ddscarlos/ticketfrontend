...
export class ModalTrazabilidadComponent implements OnInit {
  titulopant : string = "TRAZABILIDAD TICKET ";
  tkt_id : string = '0';
  tkt_fectkt : string = '';
  tkt_hortkt : string = '';
  tkt_numero : string = '';
  ...
  ProcesarRegistro() {
    const dataPost = {
      ...
      p_cer_usureg:parseInt(localStorage.getItem("usuario"))
    };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.api.getticketscer(dataPost).subscribe((data: any) => {
          if(data[0].error == 0){
            Swal.fire({
              title: 'Exito',
