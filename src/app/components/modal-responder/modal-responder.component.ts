...
@Component({
  ...
  styleUrls: ['./modal-responder.component.css']
})
export class ModalResponderComponent implements OnInit {
  titulopant : string = "RESPONDER TICKET ";
  tkt_id : string = '0';
  tkt_fectkt : string = '';
  tkt_hortkt : string = '';
  tkt_numero : string = '';
  ...
  ProcesarRegistro() {
    const dataPost = {
      ...
      p_res_usureg:parseInt(localStorage.getItem("usuario"))
    };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.api.getticketsres(dataPost).subscribe((data: any) => {
          if(data[0].error == 0){
            Swal.fire({
              title: 'Exito',
